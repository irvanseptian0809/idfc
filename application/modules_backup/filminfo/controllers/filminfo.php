...
class filminfo extends CI_Controller{
	...
	function index($text="0",$paid="P",$length="L",$genre="G"){
		...
		$data['still_in_cinema'] = $this->db->query('SELECT film_list.* FROM film_list_stillincinema INNER JOIN film_list ON film_list_stillincinema.film_list_id = film_list.id '.$where);
		$director_terkait = "";
		$id_not_in = "";
