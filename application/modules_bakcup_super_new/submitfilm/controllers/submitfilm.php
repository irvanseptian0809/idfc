...
class submitfilm extends CI_Controller{
	function submitfilm(){
		parent::__construct();
	}

	function index(){
		$data["content"] = "submitfilm/submitfilm";
		...
		if(sessionValue("id")<>""){
			...
			$this->db->select('*');
			$this->db->from('film_producer');
			$this->db->where('status', 'Active');
			$data['producer'] = $this->db->get();

			$this->db->select('*');
			$this->db->from('film_company');
			$this->db->where('status', 'Active');
			$data['company'] = $this->db->get();

			$this->db->select('*');
			$this->db->from('film_genre');
			$this->db->where('status', 'Active');
			$data['genre'] = $this->db->get();
