...
class filminfo extends CI_Controller{
	...
	function index($text="0",$paid="P",$length="L",$genre="G"){
		...
		$data['still_in_cinema'] = $this->db->query('SELECT DISTINCT film_list.*,film_list_stillincinema.new new FROM film_list_stillincinema INNER JOIN film_list ON film_list_stillincinema.film_list_id = film_list.id '.$where.' and film_list_stillincinema.deleted_at is null order by new asc limit 10');
		...
		$data['profile'] = $this->db->query('SELECT * FROM profiles  WHERE '.$w_alpb.' nama like "%'.$pc_data.'%" ORDER BY nama ASC limit 10');
		$data['company'] = $this->db->query('SELECT * FROM film_company WHERE title like "%'.$pc_data.'%" ORDER BY title DESC LIMIT 10');
