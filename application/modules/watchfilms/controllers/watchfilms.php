...
class watchfilms extends CI_Controller{
	...
	function index($text="0",$categories="C",$paid="P",$year="0-0",$length="L",$genre="G"){
		...
		$where_join_streaming = " AND film_shop.type = 'online streaming' and film_shop.price > 0 and film_shop.deleted_at IS NULL and film_shop.stock 1= 0";

		#$join_dvd = " INNER JOIN film_shop ON film_list.id = film_shop.film_id ";
		#$where_join_dvd = " and (film_shop.type = 'dvd' or film_shop.type = 'vcd') and film_shop.price > 0 and film_shop.deleted_at IS NULL and film_shop.qty > 0";
		$join_dvd ="";
		$where_join_dvd = "
		AND film_list.id IN
        ...
        $query = $this->db->query('select count(distinct film_id) as total from film_shop where qty > 0 and status = "Active" and deleted_at is null');
		#$query = $this->db->query('select count(*) total from film_list where harga_streaming<>"0" and status="Active"');
		foreach ($query->result() as $row){
		  $data['film_paid'] = $row->total;
		}

        $data['film_free'] = $data['film_total'] - $data['film_paid'];

		$query = $this->db->query('select count(*) total from film_list where durasi between 0 and 35 and status="Active" and deleted_at is null');
		foreach ($query->result() as $row){
		  $data['film_durasi_0_35'] = $row->total;
		}

		$query = $this->db->query('select count(*) total from film_list where durasi between 35 and 45 and status="Active" and deleted_at is null');
		foreach ($query->result() as $row){
		  $data['film_durasi_35_45'] = $row->total;
		}

		$query = $this->db->query('select count(*) total from film_list where durasi between 45 and 60 and status="Active" and deleted_at is null');
		foreach ($query->result() as $row){
		  $data['film_durasi_45_60'] = $row->total;
		}

		$query = $this->db->query('select count(*) as total from film_list where (durasi >= 60 or length(durasi) = 7) and category_id != 2 and deleted_at is null');
		foreach ($query->result() as $row){
			$data1 = $row->total;
			$query = $this->db->query('select count(distinct film_id) as total from film_shop where qty > 0 and price > 0 and deleted_at is null');
			foreach ($query->result() as $row){
				$data['film_durasi_60'] = $row->total + $data1;
			}
		}
		$data['title']		= "Watch Film";
		$data['deskripsi']	= "Watch Film";
		$data['keyword']	= "Watch Film";
	...
	function category($category="",$word="0",$text="0",$paid="P",$year="0-0",$length="L",$genre="G",$other="O"){
		...
		if($word=="NO"){
			...
			foreach($array_like as $key => $value) {
			    if($key == 0) {
					$where .= 'AND ( tfilm_list.title LIKE "'.$value.'%" ';
			    } else {
		...
		if($real_category == "2"){
			...
			$select_as = ' ,IFNULL((SELECT price FROM film_shop WHERE film_id=film_list.id AND type="online streaming" AND status="Active" AND deleted_at IS NULL AND qty > 0 ORDER BY created_at DESC LIMIT 1),0) price ';
			...
			WHERE film_shop.qty > 0
					AND film_shop.deleted_at IS NULL
			       AND film_shop.type = 'online streaming'
			       GROUP BY film_shop. id
					HAVING COUNT(film_shop.id)=1
					)
			...
			#$select_as = ",film_shop.price";
			$select_as = ' ,IFNULL((SELECT price FROM film_shop WHERE film_id=film_list.id AND ( type="dvd" or type="vcd" ) AND status="Active" AND deleted_at IS NULL AND stock != "Habis" ORDER BY created_at DESC LIMIT 1),0) price ';
		...
		$this->load->library('pagination');
	        ...
	        $jum_all = $this->db->query('SELECT count(*) as count FROM film_list '.$joinya.' '.$where)->result()[0]->count;
		...
		if($word == "0"){
			$data['listdata1'] = $this->db->query('SELECT film_list.*'.$select_as.' FROM film_list '.$joinya.' '.$join.' '.$where.' '.$where_join.' ORDER BY created_at desc LIMIT '.$start_from.', '.$limit.'');
		}else{
			$data['listdata1'] = $this->db->query('SELECT film_list.*'.$select_as.' FROM film_list '.$joinya.' '.$join.' '.$where.' '.$where_join.' ORDER BY title asc LIMIT '.$start_from.', '.$limit.'');
		}
		...
		$query = $this->db->query('select count(*) total from film_list '.$joinya.' '.$where.'');
		foreach ($query->result() as $row){
		  $data['film_total'] = $row->total;
		}
        ...
        #$query = $this->db->query('select count(*) total from film_list '.$where.' and harga_streaming<>"0"');
        #$query = $this->db->query('select count(*) total from film_list '.$where.' and harga_streaming<>"0"');
		$query = $this->db->query('select count(distinct film_id) as total from film_shop where qty > 0 and stock <> "Habis" and status = "Active" and deleted_at is null');
		foreach ($query->result() as $row){
		  $data['film_paid'] = $row->total;
		}
		...
		$query = $this->db->query('select count(*) total from film_list '.$joinya.' '.$where.' and durasi between 0 and 35');
		foreach ($query->result() as $row){
		  $data['film_durasi_0_35'] = $row->total;
		}

		$query = $this->db->query('select count(*) total from film_list '.$joinya.' '.$where.' and durasi between 35 and 45');
		foreach ($query->result() as $row){
		  $data['film_durasi_35_45'] = $row->total;
		}

		$query = $this->db->query('select count(*) total from film_list '.$joinya.' '.$where.' and durasi between 45 and 60');
		foreach ($query->result() as $row){
		  $data['film_durasi_45_60'] = $row->total;
		}

		$query = $this->db->query('select count(*) as total from film_list where (durasi >= 60 or length(durasi) = 7) and category_id != 2');
		foreach ($query->result() as $row){
			$data1 = $row->total;
			$query = $this->db->query('select count(distinct film_id) as total from film_shop where qty > 0 and price > 0');
			foreach ($query->result() as $row){
				$data['film_durasi_60'] = $row->total + $data1;
			}
		}
		$data['title'] = $data['category_name'];
		$data['deskripsi'] = $data['category_name'];
		$data['keyword'] = $data['category_name'];
	...
	function filmdata($id=""){
		...
		foreach ($query->result() as $film)
		{
			...
			if($film->category_id=='2' and $film->thumbnail_1!=''){
				...
				if(empty($film->thumbnail_1)){
					$thumb_trailer_final = $this->db->select("cover")->from("film_list")->where("id_full_film",$film->id)->order_by('id','desc')->limit(1)->get()->result();
					if(!empty($trailer_path)){
						$thumb_trailer_final = 'http://116.206.196.146/'.$thumb_trailer_final[0]->cover;
					}else{
			...
			if($film->category_id == '2'){
				if($film->trailer_path_hd != null){
					$trailer_path = 'http://116.206.196.146/'.$film->trailer_path_hd;
				}else{
					$trailer_path = $this->db->select("full_path_hd")->from("film_list")->where("id_full_film",$film->id)->order_by('id','desc')->limit(1)->get()->result();
					if(!empty($trailer_path)){
						$trailer_path = 'http://116.206.196.146/'.$trailer_path[0]->full_path_hd;
					}else{
						$trailer_path = 'http://116.206.196.146/'.$film->trailer_path_hd;
					}
				}
			...
			$embed = '<iframe width="560" height="315" src="'.base_url().'/pages/embed/'.$id.'" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>';
			$durasi = $film->durasi;
			...
			if(!empty($film->sort_detail)){
				$informasi_dasar = nl2br($film->sort_detail);
			}else{
				$informasi_dasar = nl2br($film->briefsynopsis_ind);
			}

			$streaming_price = $this->db->select("*")->from("film_shop")->where("film_id",$id)->where("type","online streaming")->get()->result();
			...
			$dvd_price = $this->db->select("*")->from("film_shop")->where("film_id",$id)->where("type","dvd")->where("stock !=","Habis")->get()->result();
			...
			$vcd_price = $this->db->select("*")->from("film_shop")->where("film_id",$id)->where("type","vcd")->where("stock !=","Habis")->get()->result();
			...
			if($streaming_price == 0 && $dvd_price == 0 && $vcd_price == 0){
				$cek_shop_movie = $this->db->select("*")->from("film_shop_movie")->where("film_id",$id)->get()->result();
				if(!empty($cek_shop_movie)){
					$trailer_path = 'http://116.206.196.146/'.$cek_shop_movie[0]->video;
				}
			}
			...
			$get_director = $this->db->select("*")->from("film_castcrew")->where("film_id",$film->id)->where("film_castcrew_position_id","69")->where("deleted_at",null)->get()->result();
            if(!empty($get_director)){
                $this->db->where('id', trim($get_director[0]->profile_id));
                $query_director = $this->db->get('profiles');
                if($query_director->num_rows() > 0){
                    foreach ($query_director->result() as $row_director){
                        $director_name = $row_director->nama;
                    }
                }else{
                    $director_name = "";
                }
			...
			$desc = substr(strip_tags($film->briefsynopsis_ind),0,185)." ...";
			$desc_eng = substr(strip_tags($film->briefsynopsis_eng),0,185)." ...";
			$sort_detail = substr(strip_tags($film->sort_detail),0,185)." ...";
			$sort_detail_eng = substr(strip_tags($film->sort_detail_eng),0,185)." ...";
			...
			$f_desc = $film->briefsynopsis_ind;
			$f_desc_eng = $film->briefsynopsis_eng;
			$f_sort_detail = $film->sort_detail;
			$f_sort_detail_eng = $film->sort_detail_eng;
		...
		echo json_encode(array("embed"=>$embed,"thumbnail"=>$thumb_trailer_final,"video"=>$trailer_path, "title"=>$title, "url_title"=>$url_title, "title_eng"=>$title_eng, "title_ori"=>$title_ori, "created_at"=>$created_at, "tahun"=>$tahun, "durasi"=>$durasi, "country"=>$country, "location"=>$location, "informasi_dasar"=>$informasi_dasar, "harga_streaming"=>$harga_streaming, "harga_dvd"=>$harga_dvd, "harga_vcd"=>$harga_vcd, "director"=>$director_name, "genre"=>$genre, "desc"=>$desc, "desc_eng"=>$desc_eng, "sort_detail"=>$sort_detail, "sort_detail_eng"=>$sort_detail_eng, "f_desc"=>$f_desc, "f_desc_eng"=>$f_desc_eng, "f_sort_detail"=>$f_sort_detail, "f_sort_detail_eng"=>$f_sort_detail_eng, "link_detail"=>$link_detail, "disediakan"=>$disediakan, "dsd_image"=>$dsd_image, "company_name"=>$company_name, "full_name"=>$full_name, "site"=>$site, "id_full_film"=>$id_full_film, "title_trailer"=>$title_trailer, "id_trailer"=>$id_trailer, "category_id"=>$category_id,"streaming_price"=>$streaming_price,"dvd_price"=>$dvd_price,"vcd_price"=>$vcd_price, "archive_id"=>$archive_id,"company_id"=>$company_id,"source"=>$source));
	}
	...
	function see_more($category="",$word="0",$text="0",$paid="P",$year="0-0",$length="L",$genre="G",$other="O"){
		...
		if(isset($_POST["no"]) && !empty($_POST["no"])){
			...
			if($word == "0"){
				$listdata1 = $this->db->query('SELECT film_list.*'.$select_as.' FROM film_list '.$joinya.'  '.$join.' '.$where.' '.$where_join.' ORDER BY created_at desc LIMIT '.$start.', '.$limit.'');
			}else{
				$listdata1 = $this->db->query('SELECT film_list.*'.$select_as.' FROM film_list '.$joinya.'  '.$join.' '.$where.' '.$where_join.' ORDER BY title asc LIMIT '.$start.', '.$limit.'');
			}
			...
			<div class="col-md-12" style="padding-left: 0px;padding-right: 0px;">
						  ...
						  $cover = "http://116.206.196.146/".$row->cover."";
                                ...
                                <div class="item" rel="item<?php echo $satu;?>" name="<?php echo $row->id?>" style="padding-left: 0px;padding-right: 10px;">
                                  <div class="img-film hover-element__initial" rel='popover' style="background:url('<?php echo $cover?>');background-size:100% auto;width:100%;height:<?php echo $height?>;">
                                  <?php if($harga <> 'FREE'){ ?>
                            ...
                            <div class="col-md-12 content-film-genre">
                              ...
                              <div class="col-md-4 content-right-genre">
                                  ...
                                  <div class="genre-detail-button">
                                      <div class="col-md-4 gapless" id="harga1-detail<?php echo $satu;?>1" style="padding-right: 5px;">
                                      ...
                                      <div class="col-md-4 gapless" id="harga1-detail<?php echo $satu;?>2" style="padding-right: 5px;">
                                      ...
                                      <div class="col-md-4 gapless" id="harga1-detail<?php echo $satu;?>3" style="padding-right: 5px;">
                        ...
                        <div id="detail<?php echo $dua;?>" class="detailin">
                            ...
                            <div class="col-md-12 content-film-genre">
                              ...
                              <div class="col-md-4 content-right-genre">
                                  ...
                                  <div class="genre-detail-button">
                                      <div class="col-md-4 gapless" id="harga1-detail<?php echo $dua;?>1" style="padding-right: 5px;">
                                          <span style="font-size:12px;font-weight:100;color:white;">Harga Streaming</span>
                                      ...
                                      <div class="col-md-4 gapless" id="harga1-detail<?php echo $dua;?>2" style="padding-right: 5px;">
                                          <span style="font-size:12px;font-weight:100;color:white;">Harga DVD</span>
                                      ...
                                      <div class="col-md-4 gapless" id="harga1-detail<?php echo $dua;?>3" style="padding-right: 5px;">
                                          <span style="font-size:12px;font-weight:100;color:white;">Harga VCD</span>
                        ...
                        <div id="detail<?php echo $tiga;?>" class="detailin">
                            ...
                            <div class="col-md-12 content-film-genre">
                                ...
                                <div class="col-md-4 content-right-genre">
                                    ...
                                    <div class="genre-detail-button">
                                        <div class="col-md-4 gapless" id="harga1-detail<?php echo $tiga;?>1" style="padding-right: 5px;">
                                            <span style="font-size:12px;font-weight:100;color:white;">Harga Streaming</span>
                                        ...
                                        <div class="col-md-4 gapless" id="harga1-detail<?php echo $tiga;?>2" style="padding-right: 5px;">
                                            <span style="font-size:12px;font-weight:100;color:white;">Harga DVD</span>
                                        ...
                                        <div class="col-md-4 gapless" id="harga1-detail<?php echo $tiga;?>3" style="padding-right: 5px;">
                                            <span style="font-size:12px;font-weight:100;color:white;">Harga VCD</span>
        	...
        	var video<?php echo $satu;?> = videojs('path-movie<?php echo $satu;?>');
			 ...
			 $("[rel='item<?php echo $satu;?>']").click(function() {
                    ...
                    $.ajax({
              		...
              		var film_category = $("#film_category").val();
                            ...
                            if(tmp.category_id == '10'){
                                if(tmp.id_full_film != '' && tmp.id_full_film != 0){
                                    $("#link-detail<?php echo $satu;?>").attr("href", "<?php echo base_url()?>detail/index/"+tmp.url_title+"/"+tmp.id_full_film);
                                }else{
                                    $("#link-detail<?php echo $satu;?>").attr('style','display:none');
                                }
                            }else{
                                $("#link-detail<?php echo $satu;?>").attr("href", "<?php echo base_url()?>detail/index/"+tmp.url_title+"/"+idfilm);
                            }

                            if(film_category == 2){
                                $("#harga1-detail<?php echo $satu;?>2").attr('style','display:none');
                                $("#harga1-detail<?php echo $satu;?>3").attr('style','display:none');
                            }else if(film_category == 21){
                                $("#harga1-detail<?php echo $satu;?>1").attr('style','display:none');
                                if(tmp.dvd_price == 0){
                                    $("#harga1-detail<?php echo $satu;?>2").attr('style','display:none');
                                }if(tmp.vcd_price == 0){
                                    $("#harga1-detail<?php echo $satu;?>3").attr('style','display:none');
                            ...
                            if(tmp.category_id == '6' || tmp.category_id == '8' || tmp.category_id == '4'){
                                $("#link-detail<?php echo $satu;?>").attr('style','display:none');
                            }else{
                                //$("#link-detail<?php echo $satu;?>").attr('href', full_film);
                                //$("#link-detail<?php echo $satu;?>").attr('href', "<?php echo base_url()?>detail/index/"+tmp.title+"/"+idfilm);
                                $("#link-detail<?php echo $satu;?>").attr('style','display:block;font-size:12px;font-weight:200;color:#D30E0E;');
                            }
            ...
            $("[rel='item<?php echo $dua;?>']").click(function() {
                ...
                $.ajax({
          		...
          		var film_category = $("#film_category").val();
                        ...
                        if(tmp.category_id == '10'){
                            if(tmp.id_full_film != '' && tmp.id_full_film != 0){
                                $("#link-detail<?php echo $dua;?>").attr("href", "<?php echo base_url()?>detail/index/"+tmp.url_title+"/"+tmp.id_full_film);
                            }else{
                                $("#link-detail<?php echo $dua;?>").attr('style','display:none');
                            }
                        }else{
                            $("#link-detail<?php echo $dua;?>").attr("href", "<?php echo base_url()?>detail/index/"+tmp.url_title+"/"+idfilm);
                        }

                        if(film_category == 2){
                            $("#harga1-detail<?php echo $dua;?>2").attr('style','display:none');
                            $("#harga1-detail<?php echo $dua;?>3").attr('style','display:none');
                        }else if(film_category == 21){
                            $("#harga1-detail<?php echo $dua;?>1").attr('style','display:none');
                            if(tmp.dvd_price == 0){
                                $("#harga1-detail<?php echo $dua;?>2").attr('style','display:none');
                            }if(tmp.vcd_price == 0){
                                $("#harga1-detail<?php echo $dua;?>3").attr('style','display:none');
                        ...
                        if(tmp.category_id == '6' || tmp.category_id == '8' || tmp.category_id == '4'){
                            ...
                            //$("#link-detail<?php echo $dua;?>").attr('href', full_film);
                            //$("#link-detail<?php echo $dua;?>").attr('href', "<?php echo base_url()?>detail/index/"+tmp.title+"/"+idfilm);
                            $("#link-detail<?php echo $dua;?>").attr('style','display:block;font-size:12px;font-weight:200;color:#D30E0E;');
                        }
            ...
            $("[rel='item<?php echo $tiga;?>']").click(function() {
                ...
                $.ajax({
          		...
          		var film_category = $("#film_category").val();
                        ...
                        if(tmp.category_id == '10'){
                            if(tmp.id_full_film != '' && tmp.id_full_film != 0){
                                $("#link-detail<?php echo $tiga;?>").attr("href", "<?php echo base_url()?>detail/index/"+tmp.url_title+"/"+tmp.id_full_film);
                            }else{
                                $("#link-detail<?php echo $tiga;?>").attr('style','display:none');
                            }
                        }else{
                            $("#link-detail<?php echo $tiga;?>").attr("href", "<?php echo base_url()?>detail/index/"+tmp.url_title+"/"+idfilm);
                        }

                        if(film_category == 2){
                            $("#harga1-detail<?php echo $tiga;?>2").attr('style','display:none');
                            $("#harga1-detail<?php echo $tiga;?>3").attr('style','display:none');
                        }else if(film_category == 21){
                            $("#harga1-detail<?php echo $tiga;?>1").attr('style','display:none');
                            if(tmp.dvd_price == 0){
                                $("#harga1-detail<?php echo $tiga;?>2").attr('style','display:none');
                            }if(tmp.vcd_price == 0){
                                $("#harga1-detail<?php echo $tiga;?>3").attr('style','display:none');
                        ...
                        if(tmp.category_id == '6' || tmp.category_id == '8' || tmp.category_id == '4'){
                            ...
                            //$("#link-detail<?php echo $tiga;?>").attr('href', full_film);
                            //$("#link-detail<?php echo $tiga;?>").attr('href', "<?php echo base_url()?>detail/index/"+tmp.title+"/"+idfilm);
                            $("#link-detail<?php echo $tiga;?>").attr('style','display:block;font-size:12px;font-weight:200;color:#D30E0E;');
                        }
