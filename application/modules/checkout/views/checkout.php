        ...
        <div class="container" style="width: 100%;margin-top:50px;">
          <div class="row row-list text-center" style="padding-left:55px;padding-right:55px;">
            <div class="col-md-12 text-center">
              <div class="title-checkout">Daftar Belanja</div>
              <div class="box-checkout">
                <table class="table-checkout">
                  <tr>
                    <th width="40%" class="head-table-checkout first">Nama Barang</th>
                    <th width="10%" class="head-table-checkout">Jenis</th>
                    <th width="20%" class="head-table-checkout">Harga Satuan</th>
                    <th width="10%" class="head-table-checkout">Qty</th>
                    <th width="10%" class="head-table-checkout">Jumlah</th>
                    <th width="10%" class="head-table-checkout"></th>
                  </tr>
                  <tr>
                    <td colspan="5"><hr></td>
                  </tr>
                  ...
                  foreach ($order->result() as $orderdata){
                    ...
                    $film_id = $orderdata->film_id;
                    $film_type = $orderdata->type;
                    $price = $this->db->select('price')->from('film_shop')->where('film_id',$film_id)->like('type',$film_type)->limit(1)->get()->result();
                    $price = ($price[0]->price);
                    //exit();
                  ...
                  ?>
                  <tr class="item">
                    <td class="first"><?php echo $orderdata->title?></td>
                    <td><?php echo strtoupper($orderdata->type)?></td>
                    <td>Rp <?php echo number_format($harga,0,",",".")?></td>
                    <td>
                      <?php if($plusminus=='show'){ ?>
                      <a href="#" onclick="changecount('minus','<?php echo $orderdata->id?>')"><i class="fa fa-minus"></i></a>
                      <span style="padding-left:20px;padding-right:20px;"><?php echo $orderdata->count?></span>
                      <a href="#" onclick="changecount('plus','<?php echo $orderdata->id?>')"><i class="fa fa-plus"></i></a>
                      <?php }else{ ?>
                      <span style="padding-left:20px;padding-right:20px;"><?php echo $orderdata->count?></span>
                      <?php } ?>
                    </td>
                    <td>Rp <?php echo number_format($harga*$orderdata->count,0,",",".")?></td>
                    <td><a href="#" onclick="deletecart('<?php echo $orderdata->id?>')"><i class="fa fa-trash"></i></a></td>
                  </tr>
                  <?php
                  ...
                  <tr>
                    <th colspan="4" class="text-footer text-right">Total:</th>
                    <th colspan="2" class="total-footer">Rp <?php echo number_format($total,0,",",".")?></th>
                  </tr>
                </table>
              </div>
            </div>
            <div class="col-md-12">
              ...
              <div class="col-md-2">
                <button type="button" onclick="bayar('<?php echo $trxid?>')" class="butt butt-primary sharp">Bayar</button>
              </div>
            </div>
          </div>
        </div>
